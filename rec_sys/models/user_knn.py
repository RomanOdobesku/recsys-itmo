...
class UserKnn:
    ...
    def __init__(
        ...
        self.add_popular = add_popular
        self.model_popular = model_popular
    ...
    def predict_online(self, X: np.ndarray, N_recs: int = 10) -> np.ndarray:
        if not self.users_mapping.get(int(X[0][0])):
            reco = np.array([[]])
        else:
            reco = self.predict(self.interactions[self.interactions["user_id"] == int(X[0][0])], N_recs)[
                ["item_id"]
            ].to_numpy()

        reco = np.squeeze(reco)

        if (self.add_popular) and (self.model_popular is not None) and (len(reco) < N_recs):
            reco_popular = self.model_popular.predict(X)
            reco = np.concatenate([reco, reco_popular])
            reco_set = list()
            for r in reco:
                if r in reco_set:
                    continue
                if len(reco_set) == N_recs:
                    break
                reco_set.append(r)
            reco = np.array(reco_set)
            reco = reco[: min(len(reco), N_recs)]
