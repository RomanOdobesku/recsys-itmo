...
def generate_lightfm_recs_mapper(
    ...
    def _recs_mapper(user):
        ...
        scores_vector = model.predict(
            user_id, item_iids, user_features=user_features, item_features=item_features, num_threads=num_threads
        )
...
def avg_user_metric(
    ...
    if len(y_pred) == len(y_true) == len(user_ids):
        ...
        raise ValueError(
            f"Размерности не совпадают: " f"y_pred - {len(y_pred)}, y_true - {len(y_true)}, user_ids - {len(user_ids)}"
        )
    return avg_score
