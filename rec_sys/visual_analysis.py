...
class VisualAnalysis:
    ...
    def user_data_enrichment(self, data: pd.DataFrame, user: str, columns: list) -> pd.DataFrame:
        ...
        enriched_data = (
            data
            .merge(self.interactions_df[self.interactions_df.user_id == user]
                .groupby('item_id').size().reset_index(name='user_views'), on='item_id', how='left'
            ).fillna({'user_views': 0}).astype({'user_views': int})
            .merge(self.interactions_df
                .groupby('item_id').size().reset_index(name='total_views'), on='item_id', how='left'
            ).fillna({'total_views': 0}).astype({'total_views': int})
